...
result_text_from_state = st.session_state.transcription_result["text"]

# --- Діагностика (тимчасово) ---
st.write(f"Тип результату тексту в session_state: {type(result_text_from_state)}")
# Якщо тип не рядок, можна спробувати показати саме значення
if not isinstance(result_text_from_state, str):
     st.write(f"Неочікуване значення результату тексту: {result_text_from_state}")
# --- Кінець діагностики ---
